...
#add the high wage and low wage vars
#create categories for low, high wages and gender partitions for all variables
 mesoB<-meso
 meso$m<-NULL
 meso$f<-NULL
 meso$wtype<-NULL
...
{
 mesoB[,paste0(vars[i],"f")]<-mesoB[,vars[i]]*mesoB$f
 mesoB[,paste0(vars[i],"m")]<-mesoB[,vars[i]]*mesoB$m
}

#females
 mesoB_female<-aggregate(mesoB[,paste0(vars,"f")],list(
                                             Scenario=mesoB$Scenario,
                                             Year=mesoB$Year,
                                             wtype=mesoB$wtype
                                             ),sum)
 mesoB_female$Sector<-paste0(mesoB_female$wtype,"-female")
 mesoB_female<-mesoB_female[,c("Sector","Scenario","Year",paste0(vars,"f"))]
 mesoB_female$Sector_en<-mesoB_female$Sector
 mesoB_female$Sector_Calib<-mesoB_female$Sector
 colnames(mesoB_female)<-colnames(meso)

#males
mesoB_male<-aggregate(mesoB[,paste0(vars,"m")],list(
                                            Scenario=mesoB$Scenario,
                                            Year=mesoB$Year,
                                            wtype=mesoB$wtype
                                            ),sum)
mesoB_male$Sector<-paste0(mesoB_male$wtype,"-male")
mesoB_male<-mesoB_male[,c("Sector","Scenario","Year",paste0(vars,"m"))]
mesoB_male$Sector_en<-mesoB_male$Sector
mesoB_male$Sector_Calib<-mesoB_male$Sector
colnames(mesoB_male)<-colnames(meso)
