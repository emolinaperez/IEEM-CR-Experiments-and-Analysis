...
fp_inc_sim2 = os.path.join(dir_uf, ("##APP##-sim2Invest.inc").replace("##APP##", name_app))

#build dictionaries
#df_exp_design = {'run_id': [1], 'data': ['d1'], 'sim' : ['s1']}
df_exp_design = {'run_id': [all_runs_i], 'data': ["".join(["d",str(all_runs_i)])], 'sim' : ["".join(["s",str(all_runs_i)])]}
df_exp_design = pd.DataFrame(df_exp_design)
...
#    print("Sending command:\n\t" + comm)
    ...
    #update the string
    str_sim_scen = do_repl(str_sim_scen, dict_r_sim, {"##RUNID##": str(scen)}) #here, here here, here is the problem.
